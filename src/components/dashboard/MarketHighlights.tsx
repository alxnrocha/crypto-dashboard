    ...
    <Card className="flex-1 flex flex-col p-5">
      ...
      <div className="flex flex-col gap-4">
        {items.map((coin, index) => (
          <div
            key={coin.id}
            className="flex items-center group cursor-pointer hover:bg-[#1C2333] p-2 -mx-2 rounded-xl transition-colors"
          >
            {/* Rank Number */}
            <span className="text-[#5A657A] text-[11px] font-bold w-4">
              {index + 1}
            </span>
            ...
            <div className="flex items-center gap-6">
              <span className="text-[13px] font-bold text-white w-20 text-right">
                {formatCurrency(coin.current_price)}
              </span>
              <div
                className={`text-[12px] font-bold w-16 text-right ${isGainer ? "text-[#10B981]" : "text-[#EF4444]"}`}
              >
                {isGainer ? "+" : ""}
